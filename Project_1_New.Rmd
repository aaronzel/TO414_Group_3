...
hotel$time <- 1:nrow(hotel)
hotel %>% group_by(arrival_date_month,arrival_date_year) %>%
   summarize(is_canceled_2=sum(is_canceled_2))
```
...
correlations
#as.data.frame(correlations) %>% ggplot(aes(x=name, y=correlations)) + geom_bar(stat = 'identity')
