...
graph <- ggplot(data = as.data.frame(mean_adr_per_year), aes(x=arrival_date_year, y = adr, colour = is_canceled)) + geom_point(size = 6) + ggtitle("Mean ADR per Arrival Year") + labs(x = "Arrival Year", y="Mean ADR", caption=("As the year increases, the mean ADR for cancellations increases.")) + theme(plot.title = element_text(hjust = 0.5), plot.caption = element_text(hjust = 0.5))
mean_adr_per_year
graph
```
