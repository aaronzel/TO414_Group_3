...
#AARON START HERE
```{r}
avg_adr_cancel <- hotel[hotel$is_canceled==1, ]/hotel$adr


table_mean_adr_canceled <- tapply(hotel$adr, hotel$is_canceled, mean, na.rm = TRUE)
table_mean_adr_canceled
ddply(hotel, hotel$is_canceled, summarise, mean=mean(adr))
df <- data.frame(template=names(table_mean_adr_canceled), mean=table_mean_adr_canceled)
